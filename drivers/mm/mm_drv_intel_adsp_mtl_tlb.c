...
#elif defined(CONFIG_SOC_SERIES_INTEL_ACE1X)
...
#define HPSRAM_SEGMENTS(hpsram_ebb_quantity)        ((ROUND_DOWN((hpsram_ebb_quantity) + 31u , 32u ) / 32u) - 1u)
...
static int hpsram_ref[L2_SRAM_SIZE/SRAM_BANK_SIZE] = {0};
...
static uint32_t get_hpsram_bank_idx(uint32_t tlb_entry)
{
	return (tlb_entry * CONFIG_MM_DRV_PAGE_SIZE) / SRAM_BANK_SIZE;
}
...
int sys_mm_drv_map_page(void *virt, uintptr_t phys, uint32_t flags)
{
	...
	if (UINT_TO_POINTER(phys) == NULL) {
		...
		phys = POINTER_TO_UINT(phys_block_ptr);
	}
	...
	bank_idx = get_hpsram_bank_idx(entry_idx);
...
int sys_mm_drv_unmap_page(void *virt)
{
	...
	phys = UINT_TO_POINTER((tlb_entries[entry_idx] & TLB_PADDR_MASK) *
				CONFIG_MM_DRV_PAGE_SIZE);

	sys_mem_blocks_free_contiguous(&L2_PHYS_SRAM_REGION, phys, 1);
